/*
  # AirBear Database Schema

  This creates all the necessary tables for the AirBear PWA application.

  1. New Tables
    - `locations` - Predefined pickup/dropoff locations
    - `users` - User authentication and profile data
    - `drivers` - Driver profiles and information
    - `chariots` - Vehicle information and solar battery status
    - `rides` - Ride booking and trip data
    - `ride_items` - Items ordered during rides
    - `inventory` - Bodega inventory management
...
-- Locations table for predefined spots
...
-- Inventory table for bodega items
...
-- Drivers can read their own data and be read by users during rides
CREATE POLICY "Drivers can read own data"
  ON drivers FOR SELECT
  TO authenticated
  USING (auth.uid() = id OR EXISTS (
    SELECT 1 FROM rides WHERE driver_id = drivers.id AND user_id = auth.uid()
  ));

-- Chariots can be read by authenticated users
CREATE POLICY "Chariots are readable by authenticated users"
  ON chariots FOR SELECT
  TO authenticated
  USING (true);
...
-- Users can read their own ride items
CREATE POLICY "Users can read own ride items"
  ON ride_items FOR SELECT
  TO authenticated
  USING (EXISTS (
    SELECT 1 FROM rides WHERE id = ride_items.ride_id AND user_id = auth.uid()
  ));

-- Users can read their own t-shirt purchases
CREATE POLICY "Users can read own tshirt purchases"
  ON tshirt_purchases FOR SELECT
  TO authenticated
  USING (user_id = auth.uid());

CREATE POLICY "Users can create tshirt purchases"
  ON tshirt_purchases FOR INSERT
  TO authenticated
  WITH CHECK (user_id = auth.uid());
...
-- Insert GPS locations from spots.csv
...
('Just Items Delivery', NULL, NULL, true)
